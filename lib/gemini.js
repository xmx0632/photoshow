...
/**
 * 使用Gemini API编辑图片 (nodejs 实现版本)
 * @param {string} imagePath - 要编辑的图片路径
 * @param {string} prompt - 编辑提示词
 * @returns {Promise<string>} - 返回编辑后的图片URL或Base64编码的图片数据
 */
export async function editImageNative(imagePath, prompt) {
  try {
    // 确保API密钥存在
    if (!apiKey) {
      throw new Error("Gemini API密钥未设置，请在.env.local文件中配置GEMINI_API_KEY");
    ...
    // 创建临时文件路径
    const tempImagePath = `/tmp/gemini-edited-image-${Date.now()}.png`;
    ...
    return { imageUrl, textResponse, tempImagePath };
  } catch (error) {
    console.error("图片编辑失败:", error);
    throw new Error(`图片编辑失败: ${error.message}`);
  }
}


/**
 * 使用Gemini API编辑图片 (curl实现版本)
 * @param {string} imagePath - 要编辑的图片路径
 * @param {string} prompt - 编辑提示词
 * @returns {Promise<string>} - 返回编辑后的图片URL或Base64编码的图片数据
 */
export async function editImage(imagePath, prompt) {
  try {
    ...
    // 检查是否在服务器端
    if (typeof window !== 'undefined' || !childProcess || !fs) {
      throw new Error("curl实现方式只能在服务器端使用");
    }
    ...
    // 创建临时文件路径
    const tempImagePath = `/tmp/gemini-edited-image-${Date.now()}.png`;
    const tempRequestPath = `/tmp/gemini-request-${Date.now()}.json`;
    ...
    // 获取图片的base64编码
    // 检测系统类型并设置base64参数
    let b64Flags = "-w0"; // 默认为Linux/macOS参数
    try {
      const versionOutput = childProcess.execSync('base64 --version 2>&1', { encoding: 'utf8' });
      if (versionOutput.includes('FreeBSD')) {
        b64Flags = "--input";
      }
    } catch (error) {
      console.log("无法检测base64版本，使用默认参数");
    }

    // 获取图片的base64编码
    const imgBase64 = childProcess.execSync(`base64 ${b64Flags} "${imagePath}" 2>&1`, { encoding: 'utf8' });
    ...
    // 将请求数据写入临时文件
    fs.writeFileSync(tempRequestPath, JSON.stringify(requestData));

    // 构建curl命令
    const curlCommand = `curl -s -X POST \
      "https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash-exp-image-generation:generateContent?key=${apiKey}" \
      -H "Content-Type: application/json" \
      -d @${tempRequestPath} > /tmp/gemini-response.json`;
    ...
    if (responseJson.candidates &&
      ...
      for (const part of responseJson.candidates[0].content.parts) {
        ...
        // 处理图像数据部分
          ...
          imageData = part.inlineData.data; // 这是Base64编码的图像数据
          imageUrl = `data:${part.inlineData.mimeType};base64,${imageData}`;
          console.log("成功生成编辑后的图像数据");

          // 将Base64数据保存为图片文件
          fs.writeFileSync(tempImagePath, Buffer.from(imageData, 'base64'));
          console.log(`编辑后的图片已保存到: ${tempImagePath}`);
        }
      }
    }
    ...
    // 清理临时请求文件
    try {
      fs.unlinkSync(tempRequestPath);
    } catch (error) {
      console.log("清理临时请求文件失败:", error);
    }
    ...
    throw new Error(`图片编辑失败: ${errorMessage}`);
  }
}
...
if (typeof window === 'undefined' && require.main === module) {
  (async () => {
    try {
      console.log("开始测试图片编辑功能...");
      const result = await editImage(
        "./gemini-native-image.png",
        "This is a picture. Can you add a llama next to the main subject?"
      );
      console.log("编辑成功!");
      console.log("文本响应:", result.textResponse);
      console.log("图片保存路径:", result.tempImagePath);
    } catch (error) {
